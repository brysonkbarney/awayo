...
The Screen Snapshot scene captures each display right before Awayo Lock appears and uses that image as the lock background with a tiny unlock control near the bottom. Move the pointer to the bottom edge to reveal the unlock control. macOS may ask for Screen Recording permission the first time this scene is used. Because early builds are ad-hoc signed, the installer refreshes Screen Recording permission records when the app signature changes. The NICE TRY camera gag is off by default and can be enabled in Settings; it shows a visible countdown before using the camera, and macOS may ask for Camera permission the first time.
