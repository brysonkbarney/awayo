...
- Optionally enable an instant Screen Snapshot NICE TRY camera snap and Polaroid-style card.
...
The Screen Snapshot scene captures each display right before Awayo Lock appears and uses that image as the lock background with a tiny unlock control near the bottom. Move the pointer to the bottom edge to reveal the unlock control. Awayo asks for Screen Recording permission when you choose Screen Snapshot in Settings. Early builds are ad-hoc signed with a stable local requirement so macOS can keep privacy permissions across updates. The first update from older builds may ask for Screen Recording and Camera again once. The NICE TRY camera gag is off by default and can be enabled in Settings; Awayo asks for Camera permission when you enable that setting.
...
The packaging script builds a release binary, creates `dist/Awayo.app`, and ad-hoc signs it with a stable local requirement for early testing.
